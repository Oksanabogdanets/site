...
T = [
 ...
 ('РЕЗУЛЬТАТ 10+ ТЫС. ПОДПИСЧИКОВ', 'РЕЗУЛЬТАТ: 1 000 ЗАЯВОК НА МІСЯЦЬ'),
 ...
 ('и форматами контента.', 'і форматами контенту.'), ('1 час', 'Етап 2'), ('4 часа', 'Етап 2'),
 ...
 ('столько понадобится вашего времени, для создания контента на месяц', 'Від стратегічної сесії до готових відео — місяць.'),
 ('7 часов', '4 етапи'), ('в месяц', 'роботи'), ('Этапы работы', 'Етапи роботи'), ('Обсудить проект', 'Обговорити проєкт'),
...
s = s.replace("tn_text_176314310323332090'>Етап 2<", "tn_text_176314310323332090'>Етап 1<")
s = s.replace("tn_text_176314294694146400'>Етап 2<", "tn_text_176314294694146400'>Етап 3<")
...
print('Етап 4 у картці Зйомка:', s.count("tn_text_176314294699174820'>Етап 4<"))
# «Етап N» у 40px ширше за старе «2 год» — розширюємо підписи зі 100 до 130px (атрибути + CSS)
STAGE_LBL = ['176314310323332090', '176314294689685120', '176314294694146400', '176314294699174820']
for _e in STAGE_LBL:
    s = re.sub(r"(tn-elem__1558035631%s'[^>]*?data-field-width-value=\")100\"" % _e, r'\g<1>130"', s)
    s = re.sub(r"(tn-elem__1558035631%s'[^>]*?data-field-width-res-360-value=\")100\"" % _e, r'\g<1>130"', s)
stage_css = ''.join('#rec1558035631 .tn-elem[data-elem-id="%s"]{width:130px!important}' % _e for _e in STAGE_LBL)
s = s.replace('</head>', '<style>' + stage_css + '</style>\n</head>', 1)
print('етапи: підписи 130px:', s.count('width:130px!important'))
