...
# Один спільний список пунктів на всі картки: що входить — золота галочка, що ні — закреслено.
# Публікації немає ніде: Оксана — «ми не робимо публікацію» (23.09).
ITEMS = [
    'Індивідуальна стратегія: аналіз конкурентів, ЦА і ринку',
    'Сценарії з хуками — продаючі та рекламні',
    'Супровід по зйомці: образ, стилістика, кадр',
    'Монтаж роликів',
    'Рекомендації по музиці, описах і хештегах',
    'Повна зйомка: постановка голосу, підбір локації, керування зйомкою',
...
PACKAGES = [
    ...
    ('Reels під ключ зі зйомкою',
     'Повний цикл: стратегія, сценарії, зйомка з оператором, монтаж, аналітика', {1, 2, 4, 5, 6, 7, 8}, '15, 30 або 60 відео'),
]
...
def html(link):
    cards = []
    for i, (name, sub, incl, vol) in enumerate(PACKAGES):
        lis = ''.join('<li class="%s">%s</li>' % ('yes' if k + 1 in incl else 'no', t)
